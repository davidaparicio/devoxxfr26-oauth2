...
# Step 1: Install Kyverno if missing. Skip helm when the admission controller
# is already there — re-running `helm upgrade` retriggers post-upgrade hooks
# (kyverno-clean-reports Job) that can flake on ImagePullBackOff in kind.
...
# finish TLS bootstrap. Applying a policy too early returns "connect: connection
# refused" from the webhook. Wait for the service to have real endpoints.
...
# Step 2: Apply the enforced policy. Retry a few times in case the webhook
# briefly rejects us during startup (connection refused / no endpoints).
echo -e "${BLUE}2. Applying enforced Kyverno policy (no label selector)...${NC}"
for attempt in 1 2 3 4 5; do
    if out=$(kubectl apply -f k8s/kyverno-oauth2-policy-enforced.yaml 2>&1); then
        echo "$out"
        break
    fi

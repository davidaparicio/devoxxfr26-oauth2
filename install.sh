...
echo -e "  • ${YELLOW}./kyverno.sh${NC}  Auto-inject auth annotations via a Kyverno policy (opt-in: Ingress must carry label oauth2-proxy=enabled)"
echo -e "  • ${YELLOW}./enforced.sh${NC} Same as kyverno.sh but namespace-wide — every Ingress in default is mutated, no opt-out label"
